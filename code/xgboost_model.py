...
sample_weights = compute_sample_weight(
    ...
    y=Y_Train_beams[:,0] #provide your own target name
)

sc_X = StandardScaler()
X_Train = sc_X.fit_transform(X_Train)
#X_Test = sc_X.transform(X_Test)
classifier = XGBClassifier(verbosity = 2,sample_weight=sample_weights)
...
print("Average Power Loss (APL):", apl) 
...
plt.scatter(Y_Test[:, 0], best_predictions, color='blue', label='Predicted', alpha=0.3)

# Add the ideal line
plt.plot([min(Y_Test[:, 0]), max(Y_Test[:, 0])], [min(Y_Test[:, 0]), max(Y_Test[:, 0])], color='dimgray', linestyle='--', label='Ideal')

# Add title and labels
plt.title('Actual vs. Predicted Beam Indices')
